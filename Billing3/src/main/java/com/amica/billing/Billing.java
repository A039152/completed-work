...
public class Billing {
	...
	public static final String CONFIG_NAME = "Billing";
	public static final String PROP_CUSTOMERS_FILE = "Billing.customersFile";
	public static final String PROP_INVOICES_FILE = "Billing.invoicesFile";
	private LocalDate fixedPayDate;

	public Billing(Configuration config) {
		this(
				config.getString(PROP_CUSTOMERS_FILE),
				config.getString(PROP_INVOICES_FILE)
		);
		// Try reading the pay-date property
		String payDateString = config.getString("Billing.fixedPayDate", null);
		if (payDateString != null && !payDateString.isEmpty()) {
			this.fixedPayDate = LocalDate.parse(payDateString);
		}
	}
	...
	public void createCustomer(String firstName, String lastName, Terms terms) {
		...
		if (!customers.containsKey(customer.getName())) {
			...
			throw new IllegalArgumentException(
					"There is already a customer with that name: " + customer.getName());
		}
	}
	...
	public void payInvoice(int invoiceNumber) {
		...
		if (found.isPresent()) {
			...
			if (invoice.getPaidDate().isEmpty()) {
				// Use fixed date if not null, else fallback to now
				LocalDate dateToUse = (fixedPayDate != null)
						? fixedPayDate
						: LocalDate.now();
				invoice.setPaidDate(Optional.of(dateToUse));
				saveInvoices();
				synchronized(this) {
					for (Consumer<Invoice> listener : invoiceListeners) {
						listener.accept(invoice);
