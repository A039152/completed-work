...
public class TestProgram {
	 ...
	 * resolution, with timestamps taken from the {@link }.</li>
	...
	public static void test2_History() {
		...
		Ticket ticket = helpDesk.getTicketByID(1); // Retrieve ticket by ID
		Iterator<Event> history = ticket.getHistory().iterator();

		// Verify the first event (ticket creation)
		Event createdEvent = history.next();
		assertEqual(createdEvent.getNote(), "Created ticket.", "First event note is incorrect: %s.");
		assertEqual(createdEvent.getNewStatus(), Ticket.Status.CREATED,
				"First event status should be CREATED, was %s.");

		// Verify the second event (assignment)
		Event assignedEvent = history.next();
		assertEqual(assignedEvent.getNote(), "Assigned to Technician A05589, Andree.",
				"Second event note is incorrect: %s.");
		assertEqual(assignedEvent.getNewStatus(), Ticket.Status.ASSIGNED,
				"Second event status should be ASSIGNED, was %s.");
